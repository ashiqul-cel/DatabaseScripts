...
@FromDate DATETIME, @ToDate DATETIME, @SalesPointID VARCHAR(MAX), @SKUIDs VARCHAR(MAX)
AS
SET NOCOUNT ON;
...
WHERE so.OrderDate BETWEEN @FromDate AND @ToDate AND so.SalesPointID IN (SELECT Id FROM @temSalesPointIDs)
AND soi.SKUID IN (SELECT Id FROM @temSKUIDs)
...
WHERE sI.InvoiceDate BETWEEN @FromDate AND @ToDate AND si.SalesPointID IN (SELECT Id FROM @temSalesPointIDs)
...
(
	...
	TeritoryCode VARCHAR(50) NOT NULL,
	TeritoryName VARCHAR(100) NOT NULL,
	SalesPointID INT NOT NULL,
	DistributorCode VARCHAR(50) NOT NULL,
	DistributorName VARCHAR(100) NOT NULL
)
INSERT INTO @temMH
(NationalID, NationalCode, NationalName, RegionID, RegionCode, RegionName, AreaID, AreaCode, AreaName, TeritoryID, TeritoryCode, TeritoryName, SalesPointID, DistributorCode, DistributorName)
...
sp.SalesPointID SalesPointID, sp.Code DistributorCode, sp.Name DistributorName
FROM SalesPoints sp
...
WHERE s.SalesPointID IN (SELECT Id FROM @temSalesPointIDs)

SELECT
MH.NationalID, MH.NationalCode, MH.NationalName,
MH.RegionID, MH.RegionCode, MH.RegionName,
MH.AreaID, MH.AreaCode, MH.AreaName,
MH.TeritoryID, MH.TeritoryCode, MH.TeritoryName,
MH.SalesPointID, MH.DistributorCode, MH.DistributorName,
...
INNER JOIN @temMH AS MH ON MH.SalesPointID = OrderVsSales.SalesPointID

WHERE e.EntryModule=3  AND e.[Status] = 16 AND OrderVsSales.SalesPointID IN (SELECT Id FROM @temSalesPointIDs)
...
MH.TeritoryID, MH.TeritoryCode, MH.TeritoryName,
MH.SalesPointID, MH.DistributorCode, MH.DistributorName,
OrderVsSales.SRID, e.Name, OrderVsSales.[Date], e.Code;
