ALTER PROCEDURE [dbo].[Save_Daily_TPClaim_Summary_Data]
@SystemID INT, @SalesPointID INT ,@ProcessDate DATETIME
AS

DECLARE @tmpSKUCompany TABLE (SKUID INT NOT NULL, CompanyName VARCHAR(50))
INSERT INTO @tmpSKUCompany (SKUID, CompanyName)
SELECT s.SKUID, ph3.Name CompanyName
FROM SKUs AS s
		...
		BEGIN
			...
			BEGIN
				...
				(
					...
					(SELECT TOP 1 SK.SKUID FROM SPSKUs SK INNER JOIN SalesInvoiceItem SII ON SII.SKUID = SK.SKUID	WHERE SK.SPID = SP.PromotionID AND SII.InvoiceID = SI.InvoiceID) SKUID
				...
				LEFT JOIN @tmpSKUCompany AS ts ON ts.SKUID = T1.SKUID

				union 

				select TranDate,SalesPointID,SalesPointCode,SalesPointName,ChannelCode,ChannelName,srcode,srname,contactno,routecode,routename,outletcode,outletname, 
				invoiceno,PromotionID,Promotion,SlabID,SlabNo,StartDate,EndDate,TotalSales,TotalSalespcs, ClaimValue,ClaimPcs,PromoSalesValue, ts.CompanyName from
				(
					SELECT SI.InvoiceDate TranDate,SPSP.SalesPointID, S.Code SalesPointCode,S.Name SalesPointName,ch.code ChannelCode,ch.name ChannelName,
					--CASE WHEN S.DistributorCategoryID = 1 THEN 'Retailer' WHEN S.DistributorCategoryID = 2 THEN 'Wholsaler'ELSE ''END Channel,
					e.code srcode, e.name srname,s.contactno, r.code routecode,r.name routename,  c.code outletcode, c.name outletname, 
				...
				LEFT JOIN @tmpSKUCompany AS ts ON ts.SKUID = T.SKUID
				where isnull(t.totalsales,0) > 0
			END
			FETCH NEXT FROM Dates INTO @OnDate
			SET @inner_loop = @@FETCH_STATUS
		END
