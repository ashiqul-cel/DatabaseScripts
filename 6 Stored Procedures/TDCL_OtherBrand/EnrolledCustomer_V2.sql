...
--DECLARE @CLPID INT = 1, @SlabID INT = 1

SELECT
M3.Name Region, M2.Name Area, M.Name Territory,
C.code CustomerCode,C.Name CustomerName,sp.Code,sp.Name, s.Name,
(
	CASE(C.classificationId)
	WHEN 1 THEN 'WholeSale' 
	WHEN 2 THEN 'Retailer'
	WHEN 3 THEN 'Others' END
) OutletType,
C.Address1 CustomerAddress, CH.Name as ChannelName, R.Code AS RouteCode, R.name AS RouteName
--ee.Code DSRCode, ee.Name DSRName
