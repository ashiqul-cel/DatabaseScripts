...
SELECT DISTINCT CAST(RegionName AS VARCHAR), CAST(AreaName AS VARCHAR), CAST(TerritoryName AS VARCHAR), CAST(TownName AS VARCHAR)
...
SalesPointID IN (SELECT NUMBER FROM STRING_TO_INT(@SalesPointIDs)) AND
