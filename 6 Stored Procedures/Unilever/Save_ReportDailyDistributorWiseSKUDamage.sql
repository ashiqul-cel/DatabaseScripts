...
ALTER PROCEDURE [dbo].[Save_ReportDailyDistributorWiseSKUDamage]
@SystemID int,  @SalesPointID INT= NULL, @ProcessDate DATETIME
AS 
DECLARE	@OnDate DATETIME, @SalesPoint INT, @Outer_loop INT, @inner_loop INT, @couVal int
  ...
  SET NOCOUNT ON;
	...
	ELSE
  	BEGIN
    	DECLARE SalesPoints CURSOR FOR
    	SELECT SalesPointID FROM SalesPoints WHERE SystemID=@SystemID AND SalesPointID=@SalesPointID ORDER BY SalesPointID
  	END
		...
		BEGIN
			...
			ELSE
				...
				BEGIN		
					...
					IF NOT EXISTS(SELECT [Date] FROM ReportDailyDistributorWiseSKUDamage AS rdsd WHERE rdsd.SalesPointID = @SalesPoint AND CAST(rdsd.[Date] AS DATE) = @OnDate)
						BEGIN
							INSERT INTO ReportDailyDistributorWiseSKUDamage ([DATE], [TYPE], [RegionId], [RegionCode], [RegionName], [AreaId], [AreaCode], [AreaName]
							, [TerritoryID], [TerritoryCode], [TerritoryName], [SalesPointID], [SalesPointCode], [SalesPointName], [TownName], [Category], [VariantCode]
							...
							M.SalesPointID, SP.Code, SP.Name, SP.TownName, vphu.Level3Name Category, vphu.Level6Code VariantCode, vphu.Level6Name VariantName,
							...
							(
								...
								WHERE CAST(mr.MarketReturnDate AS DATE) = @OnDate AND mr.SalesPointID = @SalesPoint
							...
							(
								...
								WHERE CAST(si.InvoiceDate AS DATE) = @OnDate AND si.SalesPointID = @SalesPoint
							...
							(
								...
								WHERE isd.DistributorID = @SalesPoint AND CAST(isd.TranDate AS DATE) = @OnDate
						...
						BEGIN
							...
							SET @couVal = isnull((select count(distinct(cast(invoicedate as date)))cou from SalesInvoices AS si
											WHERE si.SalesPointID = @SalesPoint AND CAST(si.InvoiceDate AS DATE) = @OnDate),0)
							if @couVal > 1
							
							BEGIN	
								DELETE FROM ReportDailyDistributorWiseSKUDamage WHERE SalesPointID = @SalesPoint AND CAST([Date] AS DATE) = @OnDate
