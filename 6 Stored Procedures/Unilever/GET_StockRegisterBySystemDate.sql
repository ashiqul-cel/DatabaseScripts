...
--DECLARE @SalesPointIDs VARCHAR(5000) = '22', @StartDate DATETIME = '6 Oct 2021', @EndDate DATETIME = '6 Oct 2021'
	 ...
	 FROM ( 
		...
		,Al.CPStock * Al.VatPrice CPValue
		  ...
		  (
				...
				AND ClosingDate=DATEADD(Day,-1, @StartDate))AS scs ON scs.SKUID=ss.SKUID AND scs.SalesPointID=ss.SalespointID
		...
		) AS T PIVOT (MAX(Quantity) FOR ParamTypeName IN ([Sound], [Damage],[Transit], [TransitDamage], [TransitShortage],[CPStock])
	 ) AS PivotTab
	)A
	...
	LEFT OUTER JOIN
	   (
		...
		FROM (
				...
				GROUP BY  sp.TownName, s.Code, S.Name, S.CartonPcsRatio,tt.trantypeID
				,s.SKUID, st.TranTypeID, sp.SalesPointID, st.TranDate

			 ) AS T2 PIVOT (MAX(Quantity) FOR TranType IN ([TransitIncrease],[ReceiveStock],[InvoiceSales],[InvoiceSalesBonus],[CarrierDamage],[CPStockIncrease])
