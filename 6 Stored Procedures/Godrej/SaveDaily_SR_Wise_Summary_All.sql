...
CREATE PROCEDURE [dbo].[SaveDaily_SR_Wise_Summary_All]
   ...
   BEGIN
     ...
     WHILE @inner_loop = 0
	  ...
	  IF NOT EXISTS(SELECT ItemID FROM Daily_SR_Wise_Summary AS dsws WHERE SalesPointID=@SalesPoint AND SalesDate=@OnDate)
	    BEGIN 
		  INSERT INTO Daily_SR_Wise_Summary([SalesDate],[MHNodeID],[SalesPointID],[SRID],[ScheduledCallOrder],[VisitedCallOrder],[ProductiveCallOrder],[TotalLineOrdered],[OrderQuantity],[OrderVolume],[OrderValue],[ScheduledCallSales],[VisitedCallSales],[TotalLineSold],[SalesQuantity],[SalesVolume],[SalesValue],[FirstCall], [LastCall], [TimeSpentInMKT], [SpentTimeInOutlets], [NewOutletCount], [LocAccuracyCount],[NoOfBilledOutlet]) 
				...
				ISNULL(SOT.LocAccuracyCount,0) [LocAccuracyCount],ISNULL(SI.NoOfBilledOutlet,0)NoOfBilledOutlet
					   ...
					   FROM Employees AS A INNER JOIN SalesPoints AS B ON A.SalesPointID = B.SalesPointID     
				INNER JOIN SalesPointMHNodes AS spm ON spm.SalesPointID = B.SalesPointID     
					   WHERE A.SalesPointID=@SalesPoint
					   GROUP BY A.SalesPointID, spm.NodeID, A.EmployeeID) A
				LEFT JOIN(
				...
				LEFT JOIN SKUs AS s ON s.SKUID = soi.SKUID INNER JOIN Sections AS s2 ON s2.SectionID = so.SectionID
					   WHERE so.OrderDate = @OnDate AND so.SalesPointID=@SalesPoint GROUP BY So.SalesPointID,So.SRID) SO
					   ON A.SalesPointID = So.SalespointID AND A.SRID = SO.SRID
				LEFT JOIN(
					   SELECT SO.SalespointID,SO.SRID, min(SO.CheckInTime) FirstCall, max(SO.CheckOutTime) LastCall, DATEDIFF(ss,min(SO.CheckInTime), max(SO.CheckOutTime)) / CAST(60 AS DECIMAL) TimeSpentInMKT,
						...
						FROM Salesorders SO		
					   WHERE SO.OrderDate = @OnDate AND SO.SalesPointID=@SalesPoint GROUP BY SO.SalesPointID,SO.SRID) SOT
					   ON A.SalesPointID = SOT.SalespointID AND A.SRID = SOT.SRID
				LEFT JOIN(
					   ...
					   WHERE SO2.OrderDate = @OnDate AND SO2.SalesPointID=@SalesPoint AND SO2.NoOrderReasonID is null GROUP BY SO2.SalesPointID,SO2.SRID) STO
					   ON A.SalesPointID = STO.SalespointID AND A.SRID = STO.SRID
				...
				LEFT JOIN(
					   ...
					   WHERE si.InvoiceDate = @OnDate AND si.SalesPointID=@SalesPoint GROUP BY SI.SalesPointID,SI.SRID) SI 
					   ON A.SalesPointID = Si.SalespointID AND A.SRID = SI.SRID 
				LEFT JOIN (SELECT s.SalesPointID, s.SRID,COUNT(C.CustomerID) ScheduledCallOrder 
				...
				INNER JOIN Sections AS s ON s.RouteID = r.RouteID 
							...
							GROUP BY s.SalesPointID, s.SRID) SCO on SCO.SalesPointID=A.SalesPOintID AND SCO.SRID=A.SRID
				LEFT JOIN (SELECT s.SalesPointID, s.SRID,COUNT(C.CustomerID) ScheduledCallSales 
							FROM dbo.Customers C INNER JOIN [Routes] AS r ON r.RouteID = C.RouteID 
							INNER JOIN Sections AS s ON s.RouteID = r.RouteID 
