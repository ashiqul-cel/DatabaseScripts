...
A.Quantity, A.Status, B.Code AS SKUCode, B.Name AS SKUName, B.[Status] AS SKUStatus, C.Name AS UnitName, B.IsNonSaleable, A.BookedQty
FROM dbo.SKUBatchStocks AS A
INNER JOIN dbo.SKUs AS B ON A.SKUID = B.SKUID 
INNER JOIN dbo.Units AS C ON B.UnitID = C.UnitID
INNER JOIN dbo.Brands D on B.BrandID = D.BrandID
